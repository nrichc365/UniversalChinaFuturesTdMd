...
int axapi::Strategy::setAPI(axapi::MarketQuotationAPI *in_pMarketQuotation, axapi::TradeAPI *in_pTrade)
{
    ...
    {
        sprintf_s(t_strLog, 500, "%s:TdAPI&MdAPI is NULL", t_strLogFuncName);
...
void axapi::Strategy::saveData()
{
    ...
    */
    ...
    {
        sprintf_s(t_strLog, 500, "m_hashAllOrder[%s]:OrderID(%s),OrderRef(%s),OrderType(%c),HoldTradeID(%s),updateOrderRoundSequence(%d)", i->first.c_str(), i->second.OrderID, i->second.OrderRef, i->second.OrderType, i->second.HoldTradeID, i->second.updateOrderRoundSequence);
    ...
    {
        sprintf_s(t_strLog, 500, "m_hashConfirmedOrder[%s]:Sequence(%d)", i->first.c_str(), i->second);
        t_pDataIntoFiles->writeData2File(t_strLog);
    }
    /// 记录m_vConfirmedOrder
    ...
    {
        sprintf_s(t_strLog, 500, "m_hashConfirmedHoldTrade[%s]:Sequence(%d)", i->first.c_str(), i->second);
        t_pDataIntoFiles->writeData2File(t_strLog);
    }
    ...
    {
        sprintf_s(t_strLog, 500, "m_hashUnpairedOffsetOrder[%s]:TradeID(%s),OrderRef(%s),OffsetOrderType(%c)", i->first.c_str(), i->second.TradeID, i->second.OrderRef, i->second.OffsetOrderType);
        t_pDataIntoFiles->writeData2File(t_strLog);
    }
}
...
void axapi::Strategy::updateOrderInfo()
{
    ...
    {
        sprintf_s(t_strLog, "updateOrderInfo:%s", e.what());
        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
    }
...
void axapi::Strategy::updateTradeInfo()
{
    ...
    {
        sprintf_s(t_strLog, "updateTradeInfo:%s", e.what());
        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
    }
...
void axapi::Strategy::strategyOrder()
{
    ...
    {
        sprintf_s(t_strLog, "strategyOrder:%s", e.what());
        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
    }
...
void axapi::Strategy::strategyOffset()
{
    ...
    {
        sprintf_s(t_strLog, "strategyOffset:%s", e.what());
        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
    }
...
#pragma region
                        ...
                        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
                        sprintf_s(t_strLog, "%s:m_vConfirmedOrder[%d]:%d,%s|%s|%s|%s:%s:%s", t_strLogFuncName, i, t_OrderInsertTime, m_vConfirmedOrder[i].OrderID, m_vConfirmedOrder[i].OrderRef, m_vConfirmedOrder[i].InsertTime, t_OrderInfoHour, t_OrderInfoMinutes, t_OrderInfoSeconds);
                        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
                        sprintf_s(t_strLog, "%s:curMinutes:%d,t_OrderInsertTime:%d", t_strLogFuncName, curMinutes, t_OrderInsertTime);
                        LOG4CPLUS_DEBUG(m_objLogger, t_strLog);
